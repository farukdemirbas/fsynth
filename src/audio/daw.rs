...
impl Daw {
    ...
    pub fn create_dummy_instruments(sample_rate: usize) -> Vec<Instrument> {
        let instrument1 = Instrument::new("inst1", Oscillator::new(sample_rate), 0.1, 440.0, 0.0);
        let instrument2 = Instrument::new("inst2", Oscillator::new(sample_rate), 0.1, 425.0, 0.0);
...
impl Instrument {
    ...
    pub fn apply_self(&mut self, frame: &mut [f32]) {
        let val = self.oscillator.next_value();
        let pan = (self.pan + 1.0) / 2.0; // Converting [-1..1] to [0..1]
        frame[0] += self.master_volume * val * (1.0 - pan);
        frame[1] += self.master_volume * val * pan;
    }
}
